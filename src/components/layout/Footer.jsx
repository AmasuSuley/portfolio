    ...
    <footer className="border-t border-line bg-surface/40">
      <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-8 px-6 py-12 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-3 text-xs text-accent">{site.now}</p>
          <ViewerCount className="mt-4" />
        </div>
      ...
      <div className="border-t border-line">
        <div className="mx-auto flex max-w-6xl flex-col gap-2 px-6 py-5 text-sm text-muted sm:flex-row sm:items-center sm:justify-between">
          <p>
            © {new Date().getFullYear()} {site.name}. All rights reserved.
          </p>
          <a href={site.callUrl} className="hover:text-accent">
            {site.phone}
          </a>
        </div>
      </div>
